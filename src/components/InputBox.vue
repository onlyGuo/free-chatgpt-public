...
const send = () => {
    ...
    if (glob.settings.apiKey && glob.settings.apiHost){
        requestGPT(glob.settings.apiHost, glob.settings.apiKey, sendMsg)
    }else{
      requestGPT("https://api.icoding.ink", "sk-ovRAanFBde9124d045304A208A5F253714d32fBbVjQEMXiZ", sendMsg)
    }
...
const requestGPT = (apiHost, apiKey, sendMsg) =>{
  if(glob.settings.apiHost){
    apiHost = glob.settings.apiHost;
  }
  if(glob.settings.apiKey){
    apiKey = glob.settings.apiKey;
  }
  let url = apiHost;
  if (url.endsWith('/')){
